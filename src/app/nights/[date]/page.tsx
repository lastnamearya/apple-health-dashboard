    ...
    <main className="min-h-screen bg-ink-950 text-ink-100">
      <header className="hairline-b">
        <div className="px-8 py-6 flex items-baseline justify-between">
          <Link href="/" className="label-eyebrow hover:text-pulse transition">
            ← Aryalytics
          </Link>
          <div className="flex items-center gap-6 label-eyebrow">
            {adjacent.prev && (
              <Link
                href={`/nights/${adjacent.prev}`}
      ...
      <section className="px-8 py-10 hairline-b">
        ...
        <div className="flex items-baseline gap-8 flex-wrap">
          ...
          <div className="grid grid-cols-2 gap-x-12 gap-y-3 ml-auto tnum">
            ...
            <Stat
              label="Max"
              value={stats?.maxHr ? Math.round(stats.maxHr) : "—"}
            />
      ...
      <section className="px-8 py-10 hairline-b">
        <div className="flex items-baseline justify-between mb-6">
          <h2 className="font-display text-2xl text-ink-100 font-light">
            Heart rate{" "}
            <span className="text-ink-400 italic">through the night</span>
          </h2>
      ...
      <section className="px-8 py-10">
        ...
        <div className="grid grid-cols-4 gap-px bg-ink-800">
          <Stage
            label="DEEP"
            min={session.deepMinutes}
            total={session.totalMinutes}
            accent
          />
          <Stage
            label="REM"
            min={session.remMinutes}
            total={session.totalMinutes}
            accent
          />
          <Stage
            label="CORE"
            min={session.coreMinutes}
            total={session.totalMinutes}
          />
          <Stage
            label="AWAKE"
            min={session.awakeMinutes}
            total={session.totalMinutes}
          />
        </div>
      </section>
